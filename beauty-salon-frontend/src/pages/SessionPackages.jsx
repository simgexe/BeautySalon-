...
const SessionPackages = () => {
  ...
  useEffect(() => {
    ...
    // Müşteri filtresi
    if (filterCustomer) {
      data = data.filter((s) => s.customerId === parseInt(filterCustomer));
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v, row) => (
        <span style={{ 
          color: v === 0 ? '#dc3545' : v <= 2 ? '#ffc107' : '#28a745',
          fontWeight: 'bold'
        }}>
          {v}
        </span>
    ...
    },
    {
      ...
      render: (_, row) => (
        <span 
          style={{
            backgroundColor: row.isActive ? '#28a745' : '#6c757d',
            color: 'white',
  ...
  return (
    <Layout className={sessionStyles.sessionLayout}>
      ...
      <div className={sessionStyles.listCard}>
        <div className={sessionStyles.listHeader}>
          <h3 className={sessionStyles.listTitle}>
            Seans Paketi Listesi
          </h3>
          <span className={sessionStyles.listCount}>
